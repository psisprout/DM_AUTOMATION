...
class Checker(object):
    ...
    def floating_nets(self):
        """``[(net, element, port_index)]`` for every one-sided net."""
        out = []
...
TERM_TYPES = ("rload", "cload", "rc", "none")
...
def render_terminations(deck, floating, kind="rload", value="1T", value2="1f",
    ...
    taken = set(e.name.lower() for e in deck.elements)
    out = []
    out.append("*" + "=" * 74)
    out.append("* termination for %d node(s) nothing else connects to"
               % len(floating))
    if source:
        out.append("* source: %s" % source)
    out.append("* generated by primesim-dm-setup at %s"
               % datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    out.append("*")
    out.append("* REVIEW BEFORE USING. A one-sided net is just as often a")
    out.append("* connection someone forgot as a pin nobody uses; the port")
    out.append("* each line came from is noted so you can tell them apart.")
    out.append("*" + "=" * 74)
    if not floating:
    ...
    for net, el, idx in floating:
        ...
        if kind == "none":
            ...
            continue
        for letter, val in (("R", value), ("C", value2)):
            if kind == "rload" and letter != "R":
                continue
            if kind == "cload" and letter != "C":
                continue
            n += 1
            name = "%s%d" % (prefix if letter == "R" else prefix.replace(
                "R", "C", 1), n)
            ...
            out.append("%-14s %-28s %-8s %-8s %s"
                       % (name, net, to, val, note))
    return "\n".join(out) + "\n"
