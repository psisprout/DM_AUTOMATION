...
| `graph` | **읽은 연결성을 SVG 그림으로 출력.** config 불필요 |
...
```bash
primesim-dm graph deck.sp --skip '/proj/pdk/' -o deck.svg
```

SVG는 그 자체로 완결돼 있어서(외부 폰트·스크립트 없음) 브라우저로 열거나 보고서에
그대로 붙이면 됩니다. 밝은 테마/어두운 테마 양쪽에서 읽히게 색이 들어 있습니다.
